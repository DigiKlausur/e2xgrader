define([
  ...
  function add_toolbar() {
    ...
    $("#save_widget").prepend($("<span/>").text(name).attr("id", "nb_name"));

    div.append($("<span/>").text("e²x").addClass("toolbarbutton"));

    let items = [$("#save-notbook"), $("#run_int")];
    ...
    submit_btn.append($("<span/>").text("Submit"));
    submit_btn.append($("<i/>").addClass("fa fa-paper-plane"));
    ...
    let help = $("<button/>")
      ...
      .click(() =>
        window.open(
          Jupyter.notebook.base_url + "e2xhelp/base/html/en",
          "_blank"
        )
      );
    ...
    let kernel_indicator = $("<span/>")
      .attr("id", "e2x_kernel_indicator")
