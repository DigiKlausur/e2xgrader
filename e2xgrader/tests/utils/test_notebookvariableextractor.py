...
class TestNotebookVariableExtractor(unittest.TestCase):
    ...
    def test_extract_variables_multiple_cells(self):
        ...
        add_header_to_template(self.coursedir, path, '{{ header_variable_1 }}\n{{header_variable_2}}')
        add_footer_to_template(self.coursedir, path, '{{ footer_variable_1 }}\n{{footer_variable_2}}')

        variables = NotebookVariableExtractor().extract(path)

        assert len(variables) == 4
        for variable in ['header_variable_1', 'header_variable_2', 'footer_variable_1', 'footer_variable_2']:
            assert variable in variables, f'{variable} not found'
