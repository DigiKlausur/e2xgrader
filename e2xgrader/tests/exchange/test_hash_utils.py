...
class TestGenerateDirectoryHashFile(unittest.TestCase):
    def setUp(self):
        ...
        with open(os.path.join(self.directory, ".ipynb_checkpoints", "notebook.ipynb"), "w") as f:
    ...
    def test_generate_directory_hash_file_without_exclusions_and_sha1(self):
        generate_directory_hash_file(self.directory, method="sha1", output_file="hashes.txt")
    ...
    def test_generate_directory_hash_file_without_exclusions_and_md5(self):
        generate_directory_hash_file(self.directory, method="md5", output_file="hashes.txt")
        # Call sha1sum to check the hash codes. Make sure the output is as expected.
        expected_output = (
            ".ipynb_checkpoints/notebook.ipynb: OK\n"
            "file1.txt: OK\n"
    ...
    def test_generate_directory_hash_file_with_nonexistent_directory(self):
        with self.assertRaises(FileNotFoundError):
            generate_directory_hash_file("nonexistent_directory", output_file="hashes.txt")
    ...
    def test_generate_directory_hash_file_with_specified_output_file(self):
        ...
        generate_directory_hash_file(self.directory, method="sha1", output_file=output_file)
...
class TestTruncateHashcode(unittest.TestCase):
    def test_truncate_hashcode(self):
        ...
        self.assertEqual(truncate_hashcode(hashcode, number_of_chunks=1, chunk_size=4), "1234")
        self.assertEqual(truncate_hashcode(hashcode, number_of_chunks=2, chunk_size=3), "123-456")
        self.assertEqual(
