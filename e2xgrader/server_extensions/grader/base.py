...
class BaseHandler(IPythonHandler):
    ...
    @property
    def db_url(self):
        return self.settings['grader_db_url']

    @property
    def url_prefix(self):
        return self.settings['grader_url_prefix']

    @property
    def coursedir(self):
        return self.settings['grader_coursedir']

    @property
    def authenticator(self):
        return self.settings['grader_authenticator']

    @property
    def gradebook(self):
        gb = self.settings['grader_gradebook']
        if gb is None:
            self.log.debug("creating gradebook")
            gb = Gradebook(self.db_url, self.coursedir.course_id)
            self.settings['grader_gradebook'] = gb
        return gb
    ...
    def render(self, name, **ns):
        template = self.settings['grader_jinja2_env_ui'].get_template(name)
