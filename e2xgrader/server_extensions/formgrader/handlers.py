...
class AutogradeAll(BaseHandler):
    ...
    def get(self):
        ...
        # autograde_command = "nbgrader autograde " + assignment_id + " --cell-id \"" + str(cells) + "\" --force"
...
class GenerateNewAssignment(BaseHandler):
    @web.authenticated
    @check_xsrf
    def get(self):
        coursedir = os.getcwd()
        assignment_id = self.get_argument('assignment_id')
        notebook_id = self.get_argument('notebook_id')
        cell_id = self.get_argument('cell_id')
        cell_content = self.get_argument('cell_content')
        db_url = 'sqlite:///' + os.path.join(coursedir, 'gradebook.db')
        gb = E2X_Gradebook(db_url)
        checksum1  = gb.update_cell_content(cell_id, notebook_id, assignment_id, cell_content)
        template = 'update_autograder_template.tpl'
        self.log.info("Checksum: " + checksum1)
        self.write(json.dumps(checksum1))
...
class ExportSelectedAssignmentGradesHandler(BaseHandler):
    ...
    def get(self):
        ...
        assignment_ids = assignment_ids.split(" ")
...
default_handlers = [
    ...
    (r'/formgrader/generate_assignment/?', GenerateNewAssignment),
