...
class E2XValidator(Validator):
    def validate_extra_cell(self, cell: NotebookNode) -> Union[None, float]:
        if ((is_multiplechoice(cell) or is_singlechoice(cell)) and len(get_choices(cell)) < 1) or (
            is_attachment_cell(cell) and len(cell.get("attachments", {})) < 1
        ):
            return 0
        ...
        for cell in nb.cells:
            ...
            if utils.is_grade(cell):
                ...
                if score is not None and score < max_score:
                    failed.append(cell)
            elif self.validate_all and cell.cell_type == "code" and self.code_cell_errored(cell):
                failed.append(cell)
