define([
  ...
  var clear_cell_types = function () {
    ...
    for (i = 0; i < cells.length; i++) {
      if (
        cells[i].metadata.nbgrader !== undefined &&
        cells[i].metadata.nbgrader.hasOwnProperty("cell_type")
      ) {
  ...
  var display_cell = function (cell) {
    ...
    if (
      model.is_graded(cell) ||
      model.is_solution(cell) ||
      model.is_locked(cell)
    ) {
      if (cell.element && !cell.element.hasClass(nbgrader_cls)) {
  ...
  var load_extension = function () {
    ...
    require([
      static_path + "extracell.js",
      static_path + "nbgrader.js",
    ], function (extracell, nbgrader) {
      extramodel = extracell;
      model = nbgrader;

      CellToolbar.register_callback(
        "create_assignment.grading_options",
        create_celltype_select
      );
      CellToolbar.register_callback(
