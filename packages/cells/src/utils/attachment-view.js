...
export class AttachmentGallery extends Observer {
  ...
  getControls() {
    ...
    controls.append(
      $("<button/>")
        ...
        .append("Webcam")
        .on("click", () => new Webcam(this.model).open())
    );
    let input_button = $("<button/>")
      ...
      .append("Upload")
      .on("click", () => $("#upload-attachments")[0].click());
  ...
  open() {
    ...
    this.model
      .getAttachments()
      .forEach((attachment) => that.addThumbnail(attachment));
    dialog.modal({
      keyboard_manager: Jupyter.keyboard_manager,
